...
def get_lidar_points_on_2D_image(img_idx, calib_dir, velo_dir,
                                 im_size, FeedInPoints=None,
                                 slice_points=None,
                                 min_intensity=None):
    """ Calculates the lidar point cloud, and only return the
    points that can be projected to the image.
    ...
    :param im_size: 2 x 1 list containing the size of the image
                    to filter the point cloud [w, h]
    :param min_intensity: (optional) minimum intensity required to keep a point
    :param FeedInPoints: (optional) bool to decide if read in
                         pointckouds as input.
    :param slice_points: (optional) Input pointclouds ((N x 4), where 4 = [x,y,z,i])
    :param min_intensity: (optional) minimum intensity required to keep a point
    ...
    # Read calibration info
    frame_calib = calib_utils.read_calibration(calib_dir, img_idx)
    # Get lidar signals either from database or from input
    # We assume that Lidar signal is now in the Velodyne coordinate frame.
    if not FeedInPoints:
        x, y, z, i = calib_utils.read_lidar(velo_dir=velo_dir, img_idx=img_idx)
        pts = np.vstack((x, y, z)).T
    else:
    ...
    if not min_intensity:
        ...
        intensity_filter = i > min_intensity
        point_filter = np.logical_and(image_filter, intensity_filter)
        return point_in_im[point_filter].T
