...
from wavedata.tools.core.voxel_grid_2d_v2 import VoxelGrid2D
from wavedata.tools.core.voxel_grid import VoxelGrid
from wavedata.tools.obj_detection import obj_panoptic_utils

# Since we are using full pointclouds, we need to build a new BEV generator for
# Panoptic dataset. The original generator cannot process empty BEV
from pplp.builders import bev_generator_panoptic_3d_builder
from pplp.core.label_cluster_panoptic_utils import LabelClusterUtils
from pplp.core.mini_batch_panoptic_3d_utils import MiniBatchUtils


class Panoptic3DUtils(object):
    ...
    def __init__(self, dataset):
        ...
        self.bev_generator = bev_generator_panoptic_3d_builder.build(
        ...
        # Mini Batch Utils
        self.mini_batch_panoptic_3d_utils = MiniBatchUtils(self.dataset)
        self._mini_batch_dir = self.mini_batch_panoptic_3d_utils.mini_batch_dir
    ...
    def get_anchors_info(self, classes_name, anchor_strides, sample_name):
        ...
        anchors_info = self.mini_batch_panoptic_3d_utils.get_anchors_info(classes_name,
                                                              anchor_strides,
                                                              sample_name)
        return anchors_info
    ...
    def get_mrcnn_result(self, img_idx):
        ...
        mrcnn_result = self.mini_batch_panoptic_3d_utils.load_mrcnn_mini_batches(img_idx)
        return mrcnn_result

    def get_orientnet_result(self, img_idx):
        orientnet_result = self.mini_batch_panoptic_3d_utils.load_orientnet_result(img_idx)
    ...
    def create_sliced_voxel_grid_2d(self, sample_name, source,
        ...
        # np.savetxt("/home/boom/height_map_rotated_{0}.txt".format(sample_name), height_map)
