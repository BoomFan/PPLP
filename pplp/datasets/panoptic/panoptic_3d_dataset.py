...
from pplp.datasets.panoptic.panoptic_3d_utils import Panoptic3DUtils
...
class PanopticDataset:
    def __init__(self, dataset_config):
        ...
        # Setup utils object
        self.panoptic_3d_utils = Panoptic3DUtils(self)
    ...
    def _set_up_directories(self):
        ...
        # self.velo_dir = self._data_split_dir + '/lidar_ptclouds'
        self.velo_dir = self._data_split_dir + '/lidar_ptclouds_3D'
        # print('################### Wanrning! Now we are using the full pointcloud! ###################')
        # print('################### This should be used for AVOD method only! ###################')
    ...
    def get_cluster_info(self):
        return self.panoptic_3d_utils.clusters, self.panoptic_3d_utils.std_devs

    def get_anchors_info(self, sample_name):
        return self.panoptic_3d_utils.get_anchors_info(
            self.classes_name,
            self.panoptic_3d_utils.anchor_strides,
            sample_name)
    ...
    def load_samples(self, indices, OrientNet=False):
        ...
        for sample_idx in indices:
            ...
            if self.has_labels:
                ...
                obj_labels = self.panoptic_3d_utils.filter_labels(obj_labels)
            ...
            mrcnn_result = self.panoptic_3d_utils.get_mrcnn_result(img_idx)
            # If no pedestrian can be seen on the images, break
            if not mrcnn_result:
                print('+++++++++++++ No mrcnn_result for ', img_idx,'. load_samples, early end ++++++++++++++++')
                return []

            # mrcnn_result.item().get('keypoints'): shape = Nx17x3.    [N, 17, [x, y, 1]]
            image_mrcnn_keypoints_input = mrcnn_result.item().get('keypoints')[:, :, :2]
            ...
            stereo_calib_p2 = calib_panoptic_utils.read_calibration(self.calib_dir,
                                                                    int(sample_name)).HD_11

            point_cloud = self.panoptic_3d_utils.get_point_cloud(self.bev_source,
                                                              img_idx,
                                                              image_shape)
            ...
            if obj_labels is not None:
                ...
                label_boxes_3d = np.asarray(  # [x, y, z, l, w, h, ry]
                    [box_3d_panoptic_encoder.object_label_to_box_3d(obj_label)
                     for obj_label in obj_labels])

                label_boxes_2d = np.asarray(  # [x1, y1, x2, y2]
                    [box_3d_panoptic_encoder.object_label_to_box_2d(obj_label)
                     for obj_label in obj_labels])

                label_classes = [
                    self.panoptic_3d_utils.class_str_to_index(obj_label.type)
                    for obj_label in obj_labels]
            ...
            if OrientNet:
                orientnet_result = self.panoptic_3d_utils.get_orientnet_result(img_idx)
                label_boxes_3d = orientnet_result.item().get('boxes_3d')

            # Create BEV maps
            # print('Create BEV maps')
            bev_images = self.panoptic_3d_utils.create_bev_maps(
            ...
            # print('height_maps = ', height_maps)
            # print('density_map = ', density_map)

            bev_input = np.dstack((*height_maps, density_map))
            # print('bev_input.shape = ', bev_input.shape) # (700, 800, 6)
            ...
            sample_dict = {
                ...
                constants.KEY_IMAGE_MRCNN_KEYPOINTS_INPUT: image_mrcnn_keypoints_input,
    ...
    def next_batch(self, batch_size, shuffle=True, OrientNet=False):
        """
        ...
        # print('**************** panoptic_dataset.py , next_batch() ****************')
        ...
        if start + batch_size >= self.num_samples:
            ...
            if OrientNet:
                samples_in_batch.extend(
                    self.load_samples(np.arange(start, self.num_samples), OrientNet=OrientNet))
            else:
                samples_in_batch.extend(
            ...
            if OrientNet:
                samples_in_batch.extend(self.load_samples(np.arange(start, end), OrientNet=OrientNet))
            else:
        ...
        # print('**************** panoptic_dataset.py , next_batch() end ****************')
